    ...
    while rval:
        try:
            ...
            for face, loc in faces:
                ...
                df = DeepFace.find(face, db_path = "dataset", enforce_detection=False)

                identity = df.iloc[0]['identity'].split("/")[2]
                score = df.iloc[0]['VGG-Face_cosine']

                cv2.putText(
                    img = image_total,
                    text = f"{identity}, certainty {score:.3f}",
                    org = (x, y),
